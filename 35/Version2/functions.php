...
/**
 * Handle name field which may contain a secure tripcode (##something).
 * Returns an array with 'display_name' => the final name to store/display.
 */
function parse_name_with_tripcode(string $raw_name, string $pepper): array
{
    $display_name = $raw_name;

    // Check for ## pattern
    if (strpos($raw_name, '##') !== false) {
        [$base_name, $secret] = explode('##', $raw_name, 2);
        ...
        if ($secret !== '') {
            ...
            $display_name = $base_name . '!!' . generate_secure_trip($secret, $pepper);
        } else {
...
/**
 * Generate a secure tripcode from user secret + pepper.
 * We do a short base64 or hex substring.
 */
function generate_secure_trip(string $secret, string $pepper): string
{
    // HMAC-SHA256 with the pepper, then base64-encode, truncated to 10 chars
    $hash_raw = hash_hmac('sha256', $secret, $pepper, true);
    $hash_b64 = base64_encode($hash_raw);
    return substr($hash_b64, 0, 10);
}
...
function generate_static_thread(PDO $db, int $thread_id): void
{
    ...
    if (!$op) {
        // Thread not found or was deleted => remove existing file if any
...
function render_header(string $board_name, string $title, string $page_type = 'index'): void
{
    ...
    // Minimal CSP example; expand as needed
...
{
    echo '<footer>
        <p>All trademarks and copyrights
           on this page are owned by their respective parties.
...
function render_board_index(PDO $db, ?string $board_name, ?\PDOStatement $results = null): void
{
    ...
    // Simple example form (no captcha)
...
function render_board_index_with_array(
    ...
    // Another example form (no captcha)
    echo '<form name="post" enctype="multipart/form-data" action="chess.php" method="post">
        <input type="hidden" name="csrf_token" value="', $csrf_token, '">
        <table>
            <tr><th>Name</th><td><input type="text" name="name" required maxlength="35"></td></tr>
...
{
    global $admin_password_hash, $csrf_file;
...
/**
 * Additional function: Check if file is truly an image using GD.
 * Returns true if valid image data is found, false otherwise.
 */
function is_valid_image_gd(string $filepath): bool
{
    return (bool)@getimagesize($filepath);
}

/**
 * Additional function: Check if file is truly an MP4 (placeholder).
 */
function is_valid_mp4(string $filepath): bool
{
    // For deeper checks, parse MP4 headers. For now we trust finfo + existence.
    return file_exists($filepath);
}

/**
 * Recursively fix file/directory permissions: directories => 0755, files => 0644.
 */
function fix_permissions(string $path): void
{
    if (!is_dir($path)) {
        return;
    }
