...
function buildPost(array $post, bool $isResPage): string {
    ...
    // If this is a thread (parent post) and not on a reply page, add a "View thread" link.
...
HTML;
    ...
    // (Deletion link removed)
    $html .= "<a name=\"{$post['id']}\"></a>";
    
    if ($post["subject"] !== "") {
        $html .= "<span class=\"filetitle\">" . htmlspecialchars($post["subject"]) . "</span> ";
    }
    $html .= $post["nameblock"];
    // Removed the "No.X" link and date display
    
    if ($post['parent'] != 0 && $post["file"] != "") {
        $image_desc = htmlspecialchars($post['file_original']) . ' (' .
            $post["image_width"] . 'x' . $post["image_height"] . ', ' .
            $post["file_size_formatted"] . ')';
        $html .= "<br><a target=\"_blank\" href=\"db/{$post["file"]}\"><img title=\"{$image_desc}\" src=\"db/{$post["thumb"]}\" alt=\"{$post["id"]}\" class=\"thumb\" width=\"{$post["thumb_width"]}\" height=\"{$post["thumb_height"]}\"></a>";
    }
    
    $html .= "<blockquote>{$message}</blockquote>";
    
    if ($post['parent'] == 0 && !$isResPage && isset($post['omitted']) && $post['omitted'] > 0) {
...
function buildPostBlock(string $parent): string {
    // Simple posting form for both new threads and replies.
    $isThread = ($parent === "0");
    $formTitle = $isThread ? "Create Thread" : "Post Reply";
    $subjectField = $isThread ? <<<HTML
<tr>
    <td class="postblock" title="Optional">Subject</td>
    <td><input type="text" name="subject" size="40" maxlength="75"></td>
</tr>
HTML
    : '';
    ...
    if (TINYIB_USECAPTCHA) {
        ...
        $captchaHTML = <<<HTML
<tr>
    <td class="postblock" title="Enter the text shown"> 
        <img src="captcha_png.php?key={$captcha_key}" alt="Captcha">
    </td>
    <td>
        <input type="hidden" name="captcha_ex" value="{$captcha_expect}" />
        <input type="text" name="captcha_out" size="8" required>
    </td>
</tr>
HTML;
    }
...
<tr>
    ...
    <td><input type="file" name="file" size="35" title="Images: GIF, JPG or PNG up to 2 MB."></td>
</tr>
HTML;
    }
    ...
    <form name="postform" id="postform" action="?do=post" method="post" enctype="multipart/form-data">
        ...
        <table class="postform">
            ...
            <tr>
                ...
                <td><textarea name="message" cols="48" rows="4" placeholder="" required></textarea></td>
            </tr>
            {$captchaHTML}
            {$imageField}
            <tr>
                <td class="postblock"></td>
                <td><input type="submit" value="{$formTitle}"></td>
            </tr>
        </table>
    </form>
</div>
...
function handlePost(): void {
    ...
    $post['subject'] = isset($_POST['subject']) ? htmlspecialchars(substr($_POST['subject'], 0, 75)) : '';
    ...
    // In non-text mode, require an image when starting a thread.
